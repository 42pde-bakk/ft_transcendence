...
class ChatController < ApplicationController
	...
	def is_channel_admin
		Cleanance_level::Channel_admin <= @clearance_level and @clearance_level <= Cleanance_level::Channel_owner
	end
	def is_server_admin
		Cleanance_level::Server_admin <= @clearance_level and @clearance_level <= Cleanance_level::Server_owner
	end
	...
	def get_clearance_level
		...
		if @groupchat.owner == @current_user then return Cleanance_level::Channel_owner end
		if @groupchat.admins.find_by(user: @current_user) then return Cleanance_level::Channel_admin end
		Cleanance_level::User
	end

	def handle_password_command(arr)
		if @clearance_level == Cleanance_level::Channel_owner
			if arr[1] == "set"
	...
	def handle_ban_mute_kick_command(arr)
		...
		if is_channel_admin
			if @clearance_level == Cleanance_level::Channel_admin
